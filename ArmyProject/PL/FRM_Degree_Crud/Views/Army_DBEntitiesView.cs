...
namespace ArmyProject.PL.FRM_Degree_Crud.Views.Army_DBEntitiesView{
    public partial class Army_DBEntitiesView : XtraUserControl {
        ...
        void InitializeNavigation() {
            ...
            var fluentAPI = mvvmContext.OfType<ArmyProject.PL.FRM_Degree_Crud.ViewModels.Army_DBEntitiesViewModel>();
			            fluentAPI.BindCommand(navigationBarItemDegree_TBLCollectionView, (x, m) => x.Show(m), x => x.Modules[0]);
			fluentAPI.BindCommand(barButtonItemDegree_TBLCollectionView, (x, m) => x.Show(m), x => x.Modules[0]);
