...
namespace ArmyProject.PL.FRM_Holiday_FULL_Crud.Views.Army_DBEntitiesView{
    public partial class Army_DBEntitiesView : XtraUserControl {
        ...
        void InitializeNavigation() {
            ...
            var fluentAPI = mvvmContext.OfType<ArmyProject.PL.FRM_Holiday_FULL_Crud.ViewModels.Army_DBEntitiesViewModel>();
			            fluentAPI.BindCommand(navigationBarItemGovernorates_TBLCollectionView, (x, m) => x.Show(m), x => x.Modules[0]);
			fluentAPI.BindCommand(barButtonItemGovernorates_TBLCollectionView, (x, m) => x.Show(m), x => x.Modules[0]);
